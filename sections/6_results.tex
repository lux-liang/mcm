...
\end{tabular}
\end{table}
\noindent\textit{Note: Ranges represent plausible bounds informed by literature review; no claim of empirical identification is made. All scenario parameters are tested via sensitivity analysis (Sec.~9).}
...
$A_{\text{cap}}$, $\kappa_A$, $s(t)$ & Assumed & Surveys, expert input & Scenario analysis \\
$A^*$ (tipping point) & Derived & Eq.~\ref{eq:tipping_point} & Policy thresholds \\
$E(t)$ projected & Conditional & If assumptions hold & Recommendations \\
\bottomrule
\end{tabular}
\end{table}
