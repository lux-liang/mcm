...
The congestion ratio $\rho(t) = \theta_V A(1 + \gamma_G A) / [V_{h,\max}(1 - \rho_V A)]$ (Eq.~\ref{eq:congestion}) measures the fraction of human verification bandwidth consumed by AI-generated output. When $\rho$ exceeds $\rho_{\text{thresh}} = 0.3$, adoption decelerates as oversight quality degrades. Info Security peaks at $\rho = 0.29$ (month 36), triggering adoption slowdown; Electricians remain below 0.03 (minimal constraint due to low $A_{\text{cap}}$); Graphic Designers reach 0.34 by month 48, reflecting high selection/curation load ($\theta_{\text{sel}}/\theta_V \approx 0.6$). The divergent congestion trajectories imply that verification capacity investment ($V_{h,\max}$) is a more effective policy lever for high-adoption occupations than for physically protected ones.
...
\midrule
$\kappa_A$ (adoption rate) & factor 1.5 & factor $[1.0, 2.5]$ & McKinsey 2023~\cite{mckinsey2023ai} \\
...
\end{tabular}
\end{table}
\noindent\textit{Note: ``factor $k$'' denotes multiplicative scaling relative to baseline (i.e., $k \times$ baseline value). Ranges represent plausible bounds; no claim of empirical identification. All tested via sensitivity analysis (Sec.~9).}
...
\begin{tabular}{lccc}
\toprule
\textbf{Occupation} & \textbf{BLS $\Delta$E$^\dagger$} & \textbf{Model 90\% CI} & \textbf{Status} \\
\midrule
Information Security & +14.8\% & [+11\%, +22\%] & Inside CI \\
Electricians & +12.2\% & [+4\%, +15\%] & Inside CI \\
Graphic Designers & +5.4\% & [$-2$\%, +9\%] & Inside CI \\
\bottomrule
\end{tabular}
\end{table}
\noindent\textit{$^\dagger$BLS $\Delta$E computed as $(E_{2024} - E_{2022})/E_{2022}$: ISA $(187940-163690)/163690$; Elec $(773950-690050)/690050$; GD $(223240-211890)/211890$. Source: Table~\ref{tab:audit_bls}. CI bounds from Monte Carlo scenario uncertainty propagation ($N = 1{,}000$ LHS draws).}
