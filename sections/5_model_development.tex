...
\label{eq:congestion}
\end{equation}
where $\rho_V \in [0.1, 0.3]$ represents the \textit{verification capacity erosion rate}---the marginal reduction in effective human oversight bandwidth per unit increase in AI adoption, arising from cognitive load spillovers and attention fragmentation. Empirically, $\rho_V$ captures the phenomenon whereby sustained high-volume AI output review degrades verification quality through fatigue effects.
...
\textbf{Tipping Point Derivation.} The critical adoption threshold $A^*$ emerges analytically from the steady-state condition. Setting $dE/dt = 0$ in Equation~\ref{eq:employment_dynamics} and solving for the adoption level where employment growth vanishes yields:
\begin{equation}
A^*(t) = \frac{1}{2 \cdot \text{sub\_ratio}(t)}
\label{eq:tipping_point}
\end{equation}
This threshold demarcates the transition from augmentation ($A < A^*$, where $dE/dt > 0$) to displacement ($A > A^*$, where $dE/dt < 0$). Crucially, $A^*$ is endogenous: as workforce capability $V_s$ grows, sub\_ratio declines via Equation~\ref{eq:dynamic_subratio}, thereby shifting the tipping point rightward and extending the augmentation regime.
